...
SWATTEN_IMG = "../Swatten/Product%20Image%20All-in-One%20System%20Three%20Phase%20(HV)%20(10-20)%20kW.png"
  ...
  <style>
    ...
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .suburb-hero__bg {{
      position: absolute;
      inset: 0;
      background: url('{SWATTEN_IMG}') center/cover no-repeat;
    ...
    .suburb-trust__item svg {{ color: var(--teal, #00C9A7); flex-shrink: 0; }}
    ...
    .suburb-nav__back:hover {{ text-decoration: underline; }}
    ...
    .suburb-accred img {{ height: 52px; object-fit: contain; filter: grayscale(0.2); }}
    ...
    .suburb-footer a {{ color: rgba(255,255,255,0.6); text-decoration: none; margin: 0 8px; }}
    .suburb-footer a:hover {{ color: #fff; }}
...
<body>
  ...
  <nav class="suburb-nav">
    ...
    <a href="../index.html" class="suburb-nav__back">&#8592; Back to main site</a>
  </nav>

  <!-- Hero -->
  <section class="suburb-hero">
    <div class="suburb-hero__bg"></div>
    <div class="suburb-hero__content">
      <span class="suburb-hero__eyebrow">SAA-Accredited Battery Installation &middot; Servicing {suburb}, Sydney</span>
      <h1 class="suburb-hero__h1">{title}</h1>
      <p class="suburb-hero__sub">Get a free, no-obligation quote from HelioFlo. Federal rebates available. Same-week quotes.</p>
    </div>
  </section>
...
def main():
    ...
    # Fix Swatten image filename (has a leading space on disk)
    swatten_src = os.path.join(os.path.dirname(__file__),
                               "Swatten",
                               " Product Image All-in-One System Three Phase (HV) (10-20) kW.png")
    swatten_dst = os.path.join(os.path.dirname(__file__),
                               "Swatten",
                               "Product_Image_Swatten.png")
    if os.path.exists(swatten_src) and not os.path.exists(swatten_dst):
        import shutil
        shutil.copy2(swatten_src, swatten_dst)
        print(f"Copied Swatten image → {swatten_dst}")

    for suburb in SUBURBS:
        slug     = slugify(suburb)
        filepath = os.path.join(out_dir, f"{slug}.html")
        with open(filepath, "w", encoding="utf-8") as f:
            # Use copied image path (no leading space, no URL-encoding issues)
            html = make_page(suburb).replace(
                "../Swatten/Product%20Image%20All-in-One%20System%20Three%20Phase%20(HV)%20(10-20)%20kW.png",
                "../Swatten/Product_Image_Swatten.png"
            )
            f.write(html)
